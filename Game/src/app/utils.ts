...
export class MusicFader {
    async fadeOut(audio: HTMLAudioElement, time: number, pause = true) {
...
export function getColorOnSpectrum(value: number, minValue: number, maxValue: number, strengthPercent: number): string {
    const normalizedPoint = (value - minValue) / (maxValue - minValue);
    const hue = normalizedPoint * 270;
    const lightness = strengthPercent;
